# 策略权重装配处理
* 本章节主要目的是将数据库中的抽奖策略表strategy_rule的权重规则查询到程序中，并转换成我们想要的数据形式存储到redis中
* mysql中的数据记录
```
策略id   策略值（消耗指定的积分从对应的奖品池中抽取奖品）
100001	4000:102,103,104,105 5000:102,103,104,105,106,107 6000:107,108,109
```
* redis中的数据形式：以策略id为key，所有奖品中奖几率合计为1 将每个奖品散了成一个map，key是顺序的随机数 value是奖品id，相比上一节的key多了“_4000”
```
key : big_market_strategy_rate_table_key_100001_4000
Field	Value
0	    103
1	    102
2	    102
